...
class SentenceFragmentsOpV2 : public OpKernel {
 public:
  ...
  void Compute(::tensorflow::OpKernelContext* context) override {
    //  Declare input doc tensor

    // Iterate through all the documents and find fragments. Then record output
    // to vectors for frag_start, frag_end, frag_properties, terminal_punc_token
    // and output_row_lengths

    // Emit all output Tensors.
    const Tensor* document_tensor;
    OP_REQUIRES_OK(context, context->input("doc", &document_tensor));
    OP_REQUIRES(
        context, TensorShapeUtils::IsVector(document_tensor->shape()),
        InvalidArgument(absl::StrCat("'doc' must be a vector, got shape: ",
                                     document_tensor->shape().DebugString())));
    ...
    for (int i = 0; i < document.size(); ++i) {
      // Find fragments.
      v2::SentenceFragmenter fragmenter(document(i));
