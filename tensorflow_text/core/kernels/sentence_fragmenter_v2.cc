...
bool IsPeriodSeparatedAcronym(const absl::string_view& input, int* offset) {
  ...
  for (int i = 0; i < input.length() - 1; i += 2) {
    if (absl::ascii_isupper(input.data()[i]) && input.data()[i + 1] == '.') {
      *offset = i + 2;
      if (*offset > 2) {
        result = true;
      }
...
bool IsEmoticon(const absl::string_view& input, int* offset) {
  *offset = 0;
  static std::vector<std::string> emoticon_list = {":(:)", ":)", ":(", ":o)",
    ":]", ":3", ":>", "=]", "=)", ":}", ":^)", ":-D", ":-)))))", ":-))))",
    ":-)))", ":-))", ":-)", ">:[", ":-(", ":(", ":-c", ":c", ":-<", ":<", ":-[",
    ":[", ":{", ";(", ":-||", ":@", ">:(", ":'-(", ":'(", ":'-)", ":')", "D:<",
    ">:O", ":-O", ":-o", ":*", ":-*", ":^*", ";-)", ";)", "*-)", "*)", ";-]",
    ...
    ">:-)", "}:-)", "}:)", "3:-)", ">_>^", "^<_<", "|;-)", "|-O", ":-J", ":-&",
    ":&", "#-)", "<3", "8-)", "^_^", ":D", ":-D", "=D", "^_^;;", "O=)", "}=)",
    "B)", "B-)", "=|", "-_-", "o_o;", "u_u", ":-\\", ":s", ":S", ":-s", ":-S",
    ";*", ";-*" "=(", ">.<", ">:-(", ">:(", ">=(", ";_;", "T_T", "='(", ">_<",
    "D:", ":o", ":-o", "=o", "o.o", ":O", ":-O", "=O", "O.O", "x_x", "X-(",
    "X(", "X-o", "X-O", ":X)", "(=^.^=)", "(=^..^=)", "=^_^=", "-<@%", ":(|)",
    "(]:{", "<\\3", "~@~", "8'(", "XD", "DX"};

  for (int i = 0; i < emoticon_list.size(); ++i) {
...
Status SentenceFragmenter::FindFragments(
  ...
  for (int i_start = 0; i_start < document_.size();) {
...
FragmentBoundaryMatch SentenceFragmenter::FindNextFragmentBoundary(
    int doc_index) const {
  FragmentBoundaryMatch current_match;
  FragmentBoundaryMatch previous_match;

  for (int i = doc_index; i < document_.size(); ++i) {
...
// often.
void SentenceFragmenter::UpdateLatestOpenParenForFragment(int i_start,
                                                          int i_end) {
...
void SentenceFragmenter::FillInFragmentFields(
...
// left context.
int SentenceFragmenter::GetAdjustedFirstTerminalPuncIndex(
...
// sentence boundary.
bool SentenceFragmenter::HasUnattachableTerminalPunc(
    const FragmentBoundaryMatch& match) const {
  // Get terminal punctuation span.
  int i1 = match.first_terminal_punc_index();
  if (i1 < 0) {
...
bool SentenceFragmenter::HasCloseParen(
    const FragmentBoundaryMatch& match) const {
