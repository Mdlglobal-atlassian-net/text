...
static const char* const emoticon_test_cases[] = {":(:)", ":)", ":(", ":o)",
    ":]", ":3", ":>", "=]", "=)", ":}", ":^)", ":-D", ":-)))))", ":-))))",
    ":-)))", ":-))", ":-)", ">:[", ":-(", ":(", ":-c", ":c", ":-<", ":<", ":-[",
    ":[", ":{", ";(", ":-||", ":@", ">:(", ":'-(", ":'(", ":'-)", ":')", "D:<",
    ">:O", ":-O", ":-o", ":*", ":-*", ":^*", ";-)", ";)", "*-)", "*)", ";-]",
...
TEST(SentenceFragmenterTest, Basic) {
  ...
  v2::SentenceFragmenter fragmenter(test_input);
  std::vector<SentenceFragment> fragments;
  EXPECT_TRUE(fragmenter.FindFragments(&fragments).ok());

  // TODO: Check if spaces should be considered part of the preceding fragment
  // (should 5 or 6 be the cutoff)?
...
TEST(FragmentBoundaryMatchTest, NoStateChange) {
  ...
  EXPECT_EQ(f.print_state(), "INITIAL_STATE");
}
...
TEST(FragmentBoundaryMatchTest, BasicEllipsis) {
  ...
  EXPECT_EQ(f.print_state(), "COLLECTING_TERMINAL_PUNC");
}
...
TEST(FragmentBoundaryMatchTest, ThreeStatesWithClosigParen) {
  ...
  EXPECT_EQ(f.limit_index(), 1);
  EXPECT_EQ(f.print_state(), "COLLECTING_CLOSE_PUNC");
